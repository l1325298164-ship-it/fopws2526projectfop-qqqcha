...
public class GameManager implements PlayerInputHandler.InputHandlerCallback {
    ...
    public void update(float delta) {
        ...
        for (Trap trap : traps) {
            if (trap.isActive()) trap.update(delta);
        }
    ...
    private void revivePlayer(Player dead, Player alive) {
        ...
        if (spawn == null) spawn = new int[]{alive.getX(), alive.getY()};
        ...
        for (Player p : players) {
            ...
            for (Enemy enemy : enemies) {
                ...
                if (hit) {
                    ...
                    // 扣血（会触发 Player.takeDamage 里的红色飘字）
    ...
    private void checkExitReached() {
        ...
        if (!twoPlayerMode) {
            Player p = player;
            for (ExitDoor door : exitDoors) {
                if (!door.isLocked() && door.isActive() && door.getX() == p.getX() && door.getY() == p.getY()) {
                    door.onPlayerStep(p);
    ...
    public boolean isLevelCompletedPendingSettlement() { return levelCompletedPendingSettlement; }
    public void clearLevelCompletedFlag() { levelCompletedPendingSettlement = false; }
    ...
    public void spawnProjectile(EnemyBullet bullet) {
        ...
        else Logger.debug("Non-Boba bullet spawned: " + bullet.getClass().getSimpleName());
    }
        ...
        for (Player p : players) {
            ...
            while (keyIterator.hasNext()) {
                ...
                if (key.getX() == px && key.getY() == py) {
                    ...
                    if (combatEffectManager != null) {
                        combatEffectManager.spawnFloatingText(fx, fy + 20, "+" + ScoreConstants.SCORE_KEY, Color.CYAN);
                    }
                    break;
                }
            }
            ...
            while (heartIterator.hasNext()) {
                ...
                if (h.getX() == px && h.getY() == py) {
                    ...
                    if (itemEffectManager != null) itemEffectManager.spawnHeart(fx, fy);
                    h.onInteract(p);
                    GameEventSource.getInstance().onItemCollected("HEART");
                    heartIterator.remove();
                    p.showNotification("HEAL +10  SCORE +" + ScoreConstants.SCORE_HEART);
                    if (combatEffectManager != null) {
                        combatEffectManager.spawnFloatingText(fx, fy + 20, "+" + ScoreConstants.SCORE_HEART, Color.PINK);
                    }
                }
            }
            ...
            while (treasureIterator.hasNext()) {
                ...
                if (t.getX() == px && t.getY() == py) {
                    ...
                    if (itemEffectManager != null) itemEffectManager.spawnTreasure(fx, fy);
                    t.onInteract(p);
                    GameEventSource.getInstance().onItemCollected("TREASURE");
                    treasureIterator.remove();
                    if (combatEffectManager != null) {
                        combatEffectManager.spawnFloatingText(fx, fy + 20, "+" + ScoreConstants.SCORE_TREASURE, Color.GOLD);
                    }
                }
            }
        }
    ...
    public boolean isEnemyValidMove(int x, int y) {
        ...
        for (ExitDoor door : exitDoors) {
            if (door.getX() == x && door.getY() == y) return false;
        }
        for (var trap : traps) {
            if (trap.getX() == x && trap.getY() == y && !trap.isPassable()) return false;
        }
        return true;
    }
    ...
    private boolean isValidDoorPosition(int x, int y) {
        ...
        int height = maze.length;
        if (maze[y][x] != 0) return false;
        if (y + 1 < height && maze[y + 1][x] == 1) return true;
        if (y - 1 >= 0 && maze[y - 1][x] == 1) return true;
        if (x - 1 >= 0 && maze[y][x - 1] == 1) return true;
        if (x + 1 < width && maze[y][x + 1] == 1) return true;
        return false;
    }
    ...
    private int[] randomWallCell() {
        ...
        for (int y = BORDER_THICKNESS; y < height - BORDER_THICKNESS; y++) {
            for (int x = BORDER_THICKNESS; x < width - BORDER_THICKNESS; x++) {
                if (maze[y][x] != 0) continue;
                if (isExitDoorAt(x, y)) continue;
                if ((y + 1 < height && maze[y + 1][x] == 1) || (y - 1 >= 0 && maze[y - 1][x] == 1) || (x - 1 >= 0 && maze[y][x - 1] == 1) || (x + 1 < width && maze[y][x + 1] == 1)) {
                    return new int[]{x, y};
                }
            }
        }
    ...
    public int getScore() { return scoreManager != null ? scoreManager.getCurrentScore() : 0; }
    ...
    private void syncSinglePlayerRef() {
        if (!players.isEmpty()) player = players.get(0);
        else player = null;
    }
    ...
    public LevelResult getLevelResult() {
        if (scoreManager == null) return new LevelResult(0, 0, 0, "D", 0, 1.0f);
    ...
    private int calculateTheoreticalMaxScore() {
        ...
        maxScore += 10 * 50;
        maxScore += 3 * 100;
        maxScore += difficultyConfig.keyCount * 200;
        return maxScore;
    }
    ...
    public void restoreFromSaveData(GameSaveData saveData) {
        ...
        if (scoreManager != null) scoreManager.restoreState(saveData);
        if (player != null) {
            int targetLives = saveData.lives > 0 ? saveData.lives : difficultyConfig.initialLives;
            int currentLives = player.getLives();
            if (targetLives > currentLives) player.heal(targetLives - currentLives);
